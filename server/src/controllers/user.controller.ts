...
export class UserController {
    ...
    getAllLawyers: RequestHandler = async (req, res): Promise<void> => {
    ...
    createEditRequest: RequestHandler = async (req, res): Promise<void> => {
        try {
            const { refreshToken } = req.cookies;
            const lawyerInfoDto = plainToInstance(LawyerInfoDto, req.body);
            ...
            if (req.file) {
                ...
                try {
                    ...
                    lawyerInfoDto.photo = req.file.filename;
                } catch (error) {
            ...
            const validationErrors = await validate(lawyerInfoDto);
            ...
            const request = await this.service.createEditRequest(refreshToken, lawyerInfoDto);
