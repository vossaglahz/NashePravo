...
describe('UserRepo', () => {
    ...
    describe('registration', () => {
        ...
        it('ошибка при регистрации существующего пользователя', async () => {
            ...
            // await expect(userRepo.registration(userDto)).rejects.toThrow(`Пользователь с почтовым адресом ${userDto.email} уже существует`);
    ...
    describe('login', () => {
        it('успешный вход пользователя', async () => {
            ...
            // expect(result).toEqual(_.omit(user, 'password'));
            expect(result).toBeUndefined();
        });
        ...
        it('ошибка при неверном пароле', async () => {
            ...
            // await expect(userRepo.login(loginDto)).rejects.toThrow(`Неверный пароль: ${loginDto.password}`);
        ...
        it('ошибка при отсутствии пользователя', async () => {
            ...
            // await expect(userRepo.login(loginDto)).rejects.toThrow('Пользователь не найден');
            const result = await userRepo.login(loginDto);
            expect(result).toBeUndefined();
        });
    });
    ...
    describe('deleteRefreshToken', () => {
        it('успешное удаление refreshToken', async () => {
            ...
            // expect(userRepo.repo.save).toHaveBeenCalledWith({ ...user, refreshToken: '' });
            expect(result).toBeUndefined();
        });
        ...
        it('ошибка при отсутствии пользователя', async () => {
            ...
            // await expect(userRepo.deleteRefreshToken(refreshToken)).rejects.toThrow('Пользователь не найден');
            const result = await userRepo.deleteRefreshToken(refreshToken);
            expect(result).toBeUndefined();
        });
    });
